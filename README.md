...
3. **H₁ topological persistence is predicted to peak before χ diverges** *(theoretical prediction — not yet demonstrated computationally)*: topological loops are expected to dissolve before the cluster-size distribution becomes scale-free, providing an earlier structural warning. In the current RGG model (r_c = 1 µm), the network is too tree-like near p_c for long-lived H₁ cycles to survive the persistence threshold at either 20 µm or 50 µm box sizes. H₁ detection requires either a larger r_c (denser meshing) or experimental network images from cryo-SEM/confocal where physical cross-link topology creates genuine 1-cycles.
...
### Persistent homology
The H₁ Betti number counts topological 1-cycles (closed loops) in the network. Percolation theory predicts that mesoscale pores emerge before bulk connectivity is lost — the H₁ count should peak at an earlier time than χ diverges. In the current RGG model (r_c = 1 µm), the network is approximately tree-like near p_c and produces no long-lived H₁ cycles above the persistence threshold at 20–50 µm box sizes. Detection requires either larger r_c (denser meshing giving more short cycles) or real experimental network images from cryo-SEM or confocal microscopy where physical crosslink topology creates genuine 1-cycles inaccessible to the idealized RGG.
